...
export const getPosts = async (req, res) => {
    ...
    try {
        ...
        const total = postMessage.countDocuments({}); //fast way to get total posts 
...
// PARAMS -> /posts/132 -> id = 123
export const getPostsBySearch = async (req, res) => {
    const { searchQuery, tags } = req.query;

    try {
        const title = new RegExp(searchQuery, 'i'); //Test test TEST -> test
        const posts = await PostMessage.find({ $or: [{ title }, { tags: { $in: tags.split(",") } }] }) //{opr: [{},{}]}
        res.json({ data: posts })
...
export const likePost = async (req, res) => {
    ...
    if (!req.userId) { return res.json({ message: "Unauthenticated" }); } //user id
    ...
    if (index === -1) { //if dont have index/id
        post.likes.push(req.userId);
    }
    else { //if have then dislike
